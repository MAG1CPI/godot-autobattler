...
func _spawn_extra_projectile(enemies: Array[Node]) -> void:
  ...
  var projectile := caster.ranged_attack.execute(enemy.global_position) as Projectile
  projectile.target = enemy.global_position
  projectile.hitbox.collision_layer = caster.stats.get_collision_layer()
  projectile.hitbox.collision_mask = caster.stats.get_collision_mask()
