...
func _attack() -> void:
  ...
  if actor_unit.stats.is_melee():
    var hitbox := actor_unit.melee_attack.execute(target.global_position) as HitBox
    hitbox.damage = actor_unit.stats.get_attack_damage()
    hitbox.collision_layer = actor_unit.stats.get_collision_layer()
    hitbox.collision_mask = actor_unit.stats.get_collision_mask()
    actor_unit.animation_player.animation_finished.connect(_on_attack_hit.unbind(1), CONNECT_ONE_SHOT)
  else:
    var projectile := actor_unit.ranged_attack.execute(target.global_position) as Projectile
    projectile.target = target.global_position
    projectile.hitbox.collision_layer = actor_unit.stats.get_collision_layer()
    projectile.hitbox.collision_mask = actor_unit.stats.get_collision_mask()
...
func _on_attack_hit() -> void:
  ...
  if target.stats.health <= 0: target_died.emit()


func _on_detect_range_exited(area: Area2D) -> void:
  if not area is BattleUnit or area != target: return
