...
func get_unique_unit_count(units: Array[Unit]) -> int:
  units = units.filter(func(unit_: Unit): return unit_.stats.traits.has(self ))
...
func get_levels_bbcode(unit_count: int) -> String:
  ...
  var reached_level := unit_requirements.filter(func(requirement_: int): return unit_count >= requirement_)
  for i: int in levels:
